...
static class WardrobeEquipTests
{
    public static void Run()
    {
        ...
        static string Manifest(string id, string category, string part, string rules = "{}") =>
            $$"""{"schemaVersion":4,"id":"{{id}}","name":"{{id}}","category":"{{category}}","parts":[{"part":"{{part}}","catalog":"mods/test/a.user","prefab":"mods/test/a.pfb"}],"rules":{{rules}}} """;
        var json = Manifest("set.body", "body", "BODY",
            """{"equip":{"cloak":"set.cloak","gauntlet":"set.glove"}}""");
        ...
        var empty = WardrobeSelections.Empty();
        ...
        Check(initial.Requested[WardrobeCategory.Gauntlet] == "user.glove", "Choose mutated its input");
        ...
        Check(cancelled.Requested[WardrobeCategory.Gauntlet] == "user.glove" && cancelled.Requested[WardrobeCategory.Cloak] == "user.cloak",
        ...
        Check(rewear.Requested[WardrobeCategory.Gauntlet] == "set.glove", "Explicit rewear did not reapply the declared default");
        var switchBody = WardrobeSelections.Choose(rewear, WardrobeCategory.Body, "other.body", registry);
        Check(switchBody.Requested[WardrobeCategory.Gauntlet] == "user.glove", "Switching body lost the underlying manual choice");
        ...
        Check(wearing.Requested[WardrobeCategory.Cloak] == "set.cloak", "Missing body fallback erased saved intent");
        foreach (var invalid in new[] { json.Replace("\"schemaVersion\":4", "\"schemaVersion\":3"),
        ...
        try {
            ...
            Check(afterLoadCancel.Requested[WardrobeCategory.Cloak] == "user.cloak", "Load lost reversible cloak declaration");
