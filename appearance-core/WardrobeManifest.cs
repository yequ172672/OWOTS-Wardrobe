...
/// <summary>Version 2 contract. Parsing does not imply native hiding is supported.</summary>
public static class WardrobeManifest
{
    public static WardrobeManifestEntry Parse(string json, string source)
    {
        ...
        if (root.GetProperty("schemaVersion").GetInt32() != 2) throw new FormatException("Expected schemaVersion 2");
        ...
        {
            Fields(rules, "hideParts", "incompatibleCategories");
        ...
        var entry = new WardrobeRuleEntry(id, category, Array.AsReadOnly(parts.Select(p => p.Part).ToArray()),
            hidden.AsReadOnly(), incompatible.AsReadOnly());
